...
class YOLOApp:
    def __init__(self, root: tk.Tk):
        ...
        # same stream as cam.py
        self.stream = cv2.VideoCapture(0)

        # YOLO results — written by background thread, read by GUI loop
        self.latest_labels  = []
        self.latest_message = ""
        self.annotated      = None   # latest annotated frame from YOLO
        ...
        # load YOLO in background so window appears immediately
        threading.Thread(target=self._load_model, daemon=True).start()

        # start GUI loop right away — shows raw feed while model loads
        self._gui_loop()


    #  LOAD MODEL + START YOLO THREAD
    # -----------------------------------------------------
    ...
    #  YOLO THREAD — runs inference on demand
    # ---------------------------------------------
    def _yolo_loop(self):
        while True:
            if self.paused:
    ...
    #  GUI LOOP — reads stream directly, same
    # --------------------------------------------
    def _gui_loop(self):
        if not self.paused:
            ...
            if ret:
                # share raw frame with YOLO thread
                with self.yolo_lock:
                ...
                # show annotated frame if YOLO has produced one, else raw
    ...
    def _build_ui(self):
        ...
        feed_wrapper = tk.Frame(main_row, bg=BORDER,
                                highlightthickness=1, highlightbackground=BORDER)
        feed_wrapper.pack(side="left")

        self.canvas = tk.Canvas(feed_wrapper, width=FEED_W, height=FEED_H,
        ...
        def section(parent, title):
            ...
            tk.Label(f, text=title, font=status_font,
                     fg=ACCENT_DIM, bg=PANEL_BG).pack(anchor="w", padx=8, pady=(6, 2))
            return f

        narr_sec = section(right, "▸ NARRATION")
        self.narration_var = tk.StringVar(value="Waiting…")
        tk.Label(narr_sec, textvariable=self.narration_var,
                 font=narr_font, fg=TEXT, bg=PANEL_BG,
                 wraplength=195, justify="left").pack(anchor="w", padx=8, pady=(0, 6))

        obj_sec = section(right, "▸ DETECTED OBJECTS")
        self.objects_var = tk.StringVar(value="—")
        tk.Label(obj_sec, textvariable=self.objects_var,
                 font=label_font, fg=TEXT, bg=PANEL_BG,
                 wraplength=195, justify="left").pack(anchor="w", padx=8, pady=(0, 6))

        count_sec = section(right, "▸ STATS")
        self.count_var = tk.StringVar(value="Objects : 0\nUnique   : 0")
        tk.Label(count_sec, textvariable=self.count_var,
                 font=label_font, fg=TEXT, bg=PANEL_BG,
        ...
        tk.Label(self.root, text="model: yolo26n",
                 font=status_font, fg=TEXT_DIM, bg=BG).pack(pady=(0, 8))
